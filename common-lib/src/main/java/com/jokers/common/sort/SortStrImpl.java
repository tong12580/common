...
public class SortStrImpl implements Sort {
        ...
        while (d != 1) {
            ...
            for (int x = 0; x < d; x++) {
                for (int i = x + d; i < Array.length; i = i + d) {
                    Object temp = Array[i];
                    int j;
                    for (j = i - d; j >= 0 && Array[j].toString().compareTo(temp.toString()) > 0; j = j - d) {
                        Array[j + d] = Array[j];
                    }
                    Array[j + d] = temp;
                }
    ...
    private static void merge(Object[] Array, int left, int middle, int right) {
        ...
        while (left <= middle && mid <= right) {
            //从两个数组中选取较小的数放入中间数组
            if (Array[left].toString().compareTo(Array[mid].toString()) < 0) {
        ...
        for (int i = (lastIndex - 1) / 2; i >= 0; i--) {
            ...
            while (k * 2 + 1 <= lastIndex) {
                ...
                if (biggerIndex < lastIndex) {
                    //若果右子节点的值较大  
                    if (data[biggerIndex].toString().compareTo(data[biggerIndex + 1].toString()) < 0) {
                        //biggerIndex总是记录较大子节点的索引  
                        biggerIndex++;
                    }
                }
                //如果k节点的值小于其较大的子节点的值  
                if (data[k].toString().compareTo(data[biggerIndex].toString()) < 0) {
                    //交换他们  
